...
const _printFollowFirebaseLogs = async (firebaseAuth, progressJobId, workflowId, removeTimestamps, sdk) => new Promise((resolve, reject) => {
    const jobIdRef = new Firebase(`${_getFinalFirebaseUrl(firebaseAuth)}/${progressJobId}`);
...
const showWorkflowLogsFromFirebase = async (progressJobId, follow, workflowId, removeTimestamps, sdk) => {
    ...
    await _connectToFirebase(firebaseAuth);
    if (follow) {
        await _printFollowFirebaseLogs(firebaseAuth, progressJobId, workflowId, removeTimestamps, sdk);
    } else {
