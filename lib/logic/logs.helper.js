...
const _getFirebaseApp = async (firebaseAuth) => {
    const firebaseConfig = {
        databaseURL: firebaseAuth.url,
    };
    return initializeApp(firebaseConfig);
};

const _connectToFirebase = async (firebaseAuth) => {
    const app = _getFirebaseApp(firebaseAuth);
    const auth = getAuth(app);
    try {
        await signInWithCustomToken(auth, firebaseAuth.accessToken);
...
const _printCurrentFirebaseLogs = async (firebaseAuth, progressJobId, removeTimestamps) => new Promise((resolve, reject) => {
    // const firebaseConfig = {
    //     databaseURL: `${_getFinalFirebaseUrl(firebaseAuth)}/${progressJobId}`,
    // };
    // const app = initializeApp(firebaseConfig);
    const jobIdRef = ref(getDatabase(undefined, `${_getFinalFirebaseUrl(firebaseAuth)}/${progressJobId}`));
